...
const PictureModule = ({ closeSnap }) => {
  ...
  const handleSlideClick = (index, image) => {
    if (swiperInstance) {
      swiperInstance.slideTo(index); // Set clicked slide as active
    }
    ...
    changeFrame(index, image); // Call your frame change function
  };
  const handleSlideChange = (swiper) => {
    setActiveIndex(swiper.activeIndex);
    $(".overlay-frame").css(
      "backgroundImage",
      `url('${frameList[swiper.activeIndex].url}')`
    );
  };
  let frameList = [
    {
  ...
  const changeFrame = (id, image) => {
    // const imageUrl = e.target.dataset.url;
    $(".overlay-frame").css("backgroundImage", `url('${image}')`);
  };
  ...
  useEffect(() => {
    ...
    const image_download_popup = document.getElementById(
    ...
    if (captureButton) {
      captureButton.addEventListener("click", () => {
          ...
          frameList = [
            {
              id: 1,
              url: "Filter1.png",
              banner: "filterMob1.png",
            },
            {
              id: 2,
              url: "Filter2.png",
              banner: "filterMob2.png",
            },
            {
              id: 3,
              url: "Filter3.png",
              banner: "filterMob3.png",
            },
          ];
        }
        ...
        overlayImage.src = window
          .getComputedStyle(overlayFrame)
    ...
    <div id="app" className="relative w-full h-full mx-auto">
      ...
      >
        ...
        <div id="camera_div" className="h-full w-full">
          ...
          ></video>
          <div className="overlay-frame absolute top-0 left-0 w-full h-full pointer-events-none"></div>
        </div>
      </div>
      ...
      >
        <div
          ...
          className="relative  py-3 px-6 rounded-full shadow-lg w-full"
        >
          <button
      ...
      <button
        onClick={closeSnap}
        className=" absolute top-4 right-4  text-[#000A32] py-2 px-6 rounded-3xl z-30"
      >
      ...
      <div
        id="filterOptions"
        className="w-full absolute inset-0 overflow-x-auto  flex flex-col gap-6 items-end justify-center p-4 bg-gradient-to-l from-[#00000036] to-transparent"
      >
          ...
          >
            {frameList.map((frame, index) => (
              <SwiperSlide
                ...
                id={activeIndex === index ? "active-slide" : ""}
              >
                <img
                  // onClick={changeFrame}
                  onClick={() => handleSlideClick(index, `/${frame.url}`)}
                  src={frame.banner}
        ...
        {/* <div
            key={`${frame.id}-${index}`}
            className="cursor-pointer w-20 h-20 bg-transparent"
          >
            <img
              onClick={changeFrame}
              src={frame.banner}
